...
session_start();
unset($_SESSION['id']);
if(isset($_POST['bgantipassword'])){
	$koneksi = mysqli_connect("localhost", "root", "", "pmbni");
	//$koneksi = mysqli_connect("localhost", "root", "", "pmbni");
	$query = mysqli_query($koneksi, "select id, password from login where username='".$_POST['username']."'");
	while($data = mysqli_fetch_array($query)){
			$id = $data['id'];
			$password = $data['password'];
		}
	if($_POST['password_baru']==$_POST['konfirmasi_password']){
	$sql = "update login set password='".$_POST['password_baru']."' where username='".$_POST['username']."'";
...
<head>
	<link  href="pmbni/bni.png" rel="shortcut icon" />
	<meta charset="utf-8" />
	<title>BNI 46 Login</title>
			...
			<div class="login-content">
				...
				<form method="POST" name="">
					...
					<div class="form-group">
						...
						<input type="password" class="form-control" value="" name="password_baru" />
					</div>
					
					<div class="form-group">
						<label>Konfirmasi Password <span class="text-danger">*</span></label>
						<input type="password" class="form-control" value="" name="konfirmasi_password" />
					</div>
					...
					<div class="d-flex align-items-center">
						<button name="bgantipassword" type="submit" class="btn btn-success width-150 btn-rounded">Ganti Password</button>
						<a href="login.php" class="btn btn-primary width-150 btn-rounded">Login</a>
